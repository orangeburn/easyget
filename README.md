...
Easyget 是一个面向招标、采购、公示线索的桌面化采集与人工筛选工具。项目采用“大模型语义扩词 + 自动搜集 + 规则初筛 + 大模型语义过滤 +人工最终决策”的工作流，重点解决两个问题：

- 尽快把最新线索抓回来
- 让人工判断发生在更干净的候选集合里

利用LLM大模型根据粗略的关键词，完成语义扩词，生成更多维度的关键词进行自动化搜索；获得线索后，先根据用户自定义的条件完成硬性过滤，再利用LLM大模型完成第二层语义化过滤。确保用户不会被浩瀚的噪音信息淹没（希望如此哈哈哈，但语义过扩词和过滤流程，实在很依赖模型质量）。
它是一个把搜集、过滤、查看、导出串起来的流式工作台，并不会成为一个“全自动替你做决策”的系统，因为反爬和微信生态实在难搞。
提供本地搜索引擎调用 + search api 双模式，支持 Serper 和 Tavily（因为这两家免费额度比较高，能利用 Google 搜索引擎资源）。
本地playwright强制开启，search api 随意。
